      ...
      <div class="container-fluid">
		...
		<div id="contents">
			<div class="container">
				...
				<div class="board-title">검색</div>
					<c:if test="${!empty userid}"><div><a href="Board_Controller?command=writeform&div_num=${div_num}"><button>글 등록</button></a></div></c:if>
					<c:if test="${empty userid}"><div><a href="#"><button>로그인해주세요</button></a></div></c:if>
					<table class="table table-border" style="text-align: center; border: 1px solid; width: 100%;">
						...
						<tbody>
							...
							<c:forEach var="j" begin="1" end="${togbean.re_step()-1*5}">	
								...
								<th><a href="Board_Controller?command=boardview&num=${togbean.num}&div_num=${div_num}"style="text-align: center;">${togbean.subject}</a></th>
					...
					<span><a href="Board_Controller?command=mypet_list&div_num=${div_num}&pageNum=${startPage -5}">이전</a></span>
					</c:if>
					<c:forEach var="pagenum" begin="${startPage}" end="${EndPage}">
							
								
								
								<span><a href="Board_Controller?command=mypet_list&div_num=${div_num}&pageNum=${pagenum}">
								[${pagenum}]</a></span>
							
					</c:forEach>
					<c:if test="${EndPage < Pageconut}">			
					<span><a href="Board_Controller?command=mypet_list&div_num=${div_num}&pageNum=${startPage +5}">다음</a></span>
					</c:if>
					</div>	

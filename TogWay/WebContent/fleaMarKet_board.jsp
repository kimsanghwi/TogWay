      ...
      <div class="container-fluid">
		...
		<div id="contents">
			<div class="container">
				...
				<div class="board-title" style="text-align: center;"><img src="img/shopping-bag.svg">플리마켓</div>
				  <c:if test="${!empty userid}"><div><a href="Board_Controller?command=writeform&div_num=3"><button class="content-btn">글 등록</button></a></div></c:if>
				  <c:if test="${empty userid}"><div><a href="#"><button style="color: #000000;">로그인하세요.</button></a></div></c:if>
					...
					<span><a href="Board_Controller?command=fleamarket_list&div_num=3&pageNum=${startPage -5}">이전</a></span>
					</c:if>
					<c:forEach var="pagenum" begin="${startPage}" end="${EndPage}">
							
								
								
								<span><a href="Board_Controller?command=fleamarket_list&div_num=3&pageNum=${pagenum}">
								[${pagenum}]</a></span>
							
					</c:forEach>
					<c:if test="${EndPage < Pageconut}">			
					<span><a href="Board_Controller?command=fleamarket_list&div_num=3&pageNum=${startPage +5}">다음</a></span>
					</c:if>
					</div>	
